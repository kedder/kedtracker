...
#define spi_init()                       // SPI initialize (NOOP)
#define spi_fast    HAL_SDFast           // switch to a fast SPI clock
#define spi_slow    HAL_SDSlow           // switch to a slow SPI clock
#define spi_cs_low  HAL_SDSelect         // SPI Chip-Select control for the SD card reader
#define spi_cs_high HAL_SDDeselect       //
#define spi_txrx    HAL_SDTransferByte   // SPI byte exchange
...
static void sd_cmd(uint8_t cmd, uint32_t arg)  // send command+argument+CRC to the SD card (CS already active)
...
static uint16_t sd_cmd_r2(uint8_t cmd, uint32_t arg)
{ spi_cs_high();
  sd_idle_bytes(sd_head_bytes);
  spi_cs_low();
  sd_cmd(cmd, arg);                // send command
...
static uint8_t sd_cmd_r7(uint32_t *resp, uint8_t cmd, uint32_t arg)
{ spi_cs_high();
  sd_idle_bytes(sd_head_bytes);
  spi_cs_low();
  sd_cmd(cmd, arg);                 // send command
...
static inline uint8_t sd_cmd8(uint32_t *resp, uint32_t arg) { return sd_cmd_r7(resp, 0x08, arg); }
...
static inline uint8_t  sd_set_sector_size(uint32_t len) { return sd_cmd_r1(16, len); }
...
static inline uint8_t sd_read_csd_raw(uint8_t *data) { return sd_cmd_r1b( 9, 0, data, 16); }
// static inline uint8_t sd_read_cid_raw(uint8_t *data) { return sd_cmd_r1b(10, 0, data, 16); }

typedef struct {
   uint8_t data[16];
} sd_csd;

static uint8_t sd_csd_version(sd_csd *csd) {
  return csd->data[0];
}

static uint8_t sd_csd_capacity(sd_csd *csd) {
     uint32_t capacity;
     uint8_t *data = csd->data;
     if(sd_csd_version(csd)==1)
...
static inline uint8_t sd_read_csd(sd_csd *csd)   { return sd_read_csd_raw(csd->data); }
...
// uint8_t sd_cid_manuf_id(sd_cid *cid) const { return cid->data[0]; }


// static inline uint8_t sd_read_cid(sd_cid *cid)   { return sd_read_cid_raw(cid.data); }
...
{ card_stat = STA_NOINIT;
  ...
  spi_init();
  spi_slow();
  uint8_t ret, stat; uint32_t resp=0;
  ...
  card_type = sd_csd_version(&csd);       // 0 = SD, 1 = SDHC
  capacity  = sd_csd_capacity(&csd);
